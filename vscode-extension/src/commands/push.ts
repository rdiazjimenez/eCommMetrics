...
export async function pushCommand(output: vscode.OutputChannel, statusBar: PqSyncStatusBar): Promise<void> {
    ...
    try {
        result = run(
            'npx',
            ['tsx', scriptPath('import_mcode.ts'), config.workbookPath, config.mcodePath, '--in-place'],
            workspaceRoot(),
        );
    } catch (err: unknown) {
